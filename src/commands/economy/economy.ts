...
export class Economy {
    ...
    public async getBalance(user: string) {
        ...
        return g.economy.find(e => e.user == user) || null;
    }

    public async setBalance(user: string, amount: number) {
        let g = (await this.fetch()) as guildModel;
        let i = g.economy.findIndex(e => e.user == user) as number;
        let e = g.economy[i];

        if (!e) {
            e = { user, balance: 0 };
            i = g.economy.push(e) - 1;
        }
